...
class JointEmbeddingModel:
    ...
    def build(self):
        ...
        merged_code= Concatenate(name='code_merge',axis=1)([methname_out,api_out,tokens_out,sbt_out])
        '''
        ...
        activ_1=Activation('softmax',name='mq_AP_active_colum')
        dot_1=Dot(axes=1,normalize=False,name='mq_column_dot')
        attention_matrix = Lambda(lambda x: K.expand_dims(x, 1))(attention_out)
        conv1 = Conv2D(100,2,data_format='channels_first',padding='same', activation='relu',strides=1,name='mq_conv1')
        desc_conv = conv1(attention_matrix)
        dense_desc = Dense(30,use_bias=False,name='dense_mq_desc')
        desc_conv= dense_desc(desc_conv)
        desc_conv = gap_cnn(desc_conv)
        desc_att = activ_1(desc_conv)
        desc_out = dot_1([desc_att, merged_desc])
        # out_2 row wise
        ...
        activ_2=Activation('softmax',name='mq_AP_active_row')
        dot_2=Dot(axes=1,normalize=False,name='mq_row_dot')
        attention_matrix = Lambda(lambda x: K.expand_dims(x, 1))(attention_transposed)
        conv2 = Conv2D(100,2,data_format='channels_first',padding='same', activation='relu',strides=1,name='mq_conv2')
        code_conv = conv2(attention_matrix)
        dense_code = Dense(236,use_bias=False,name='dense_mq')
        code_conv=dense_code(code_conv)
        code_conv=gap_cnn(code_conv)
        code_att = activ_2(code_conv)
        code_out = dot_2([code_att, merged_code])
